...
#include <sys/stat.h>
#include <unistd.h>
#define FIFO1 "/snap/gedit/684/Downloads/fifo1"
#define FIFO2 "/snap/gedit/684/Downloads/fifo2"
...
// Read from FIFO1, process it
while (read(fd_read, buffer, sizeof(buffer)) > 0) {
count_char_word_line(buffer, &char_count, &word_count, &line_count);
sprintf(buffer, "Chars: %d, Words: %d, Lines: %d", char_count, word_count, line_count);
